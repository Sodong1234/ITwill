...
// 	=> 새 창 띄울 때 ID중복확인 버튼 우측 빈공간에 "중복확인완료" 표시하기
	...
	function check() {
		// 입력받은 ID 값 가져와서 4 ~ 8 자리일 경우에만 새 창 띄우기
		var id = document.fr.id.value;
			
		if (id.length >= 4 && id.length <= 8) {
			window.open("check_id.html", "check", "width=300", )
			document.getElementById("checkIdResult").innerHTML = "중복 확인 완료"
		} else { // 유효하지 않은 아이디일 경우
			alert("4~8 글자만 사용 가능합니다")
...
// 	    아니면, "비밀번호 불일치"(빨간색) 표시
	function checkConfirmPasswd(confirmPasswd) {
		...
		if (passwd == confirmPasswd) {
			...
			span_checkConfirmPasswdResult.innerHTML = "사용 불가능한 패스워드"
...
function checkJumin1(jumin1) {
	if(jumin1.length == 6) {
		document.fr.jumin2.focus(); // 커서 요청
	}
}
...
//    이메일 두번째 항목(@ 기호 뒤)에 표시하기
//    단, 직접입력 선택 시 표시된 도메인 삭제하기
	function selecteDomain(domain) {
		document.fr.email2.value = domain; 
		
		// 만약, "직접입력" 항목 선택 시 도메인 입력창(email2)에 커서 요청
		if(domain == "") { // "직접입력" 선택 시 domain 변수에 "" 값이 전달됨
		document.fr.email2.disable = false; // 입력 창 잠금 해제
// 		document.fr.email2.focus(); // 커서 요청
		} else { // 다른 도메인 선택 시
		document.fr.email2.disable = true; // 입력 창 잠금
		}
	}
...
// 	   "전체선택" 해제 시 취미 항목 모두 체크 해제하기
	function checkAll(isChecked) {
		...
		var checkAll = document.fr.check_all;

		if (isChecked) { // isChecked == true와 동일한 조건식
		...
		<table border="1">
			...
			<tr>
				...
				<td><input type="text" name="id" placeholder="4 ~ 8글자 사이 입력"
					required="required"> <input type="button" value="ID중복확인"
					onclick="check()"><span id="checkIdResult"></span></td>
			</tr>
			<tr>
				<td>비밀번호</td>
				<td><input type="password" name="passwd"
					placeholder="8 ~ 16글자 사이 입력" required="required"
					onkeyup="checkPasswd(this.value)"><span
			...
			<tr>
				...
				<td><input type="password" name="passwd2" required="required"
					...
					id="checkConfirmPasswdResult"></span></td>
			</tr>
			<tr>
				<td>주민번호</td>
				<td><input type="text" name="jumin1" required="required"
					onkeyup="checkJumin1(this.value)"> - <input type="text"
					name="jumin2" required="required" onkeyup="checkJumin2(this.value)"></td>
			</tr>
			<tr>
				<td>E-Mail</td>
				<td><input type="text" name="email1" required="required">@
					<input type="text" name="email2" required="required"> <select
					name="emailDomain" onchange="selecteDomain(this.value)">
			...
			<tr>
				...
				<td><input type="checkbox" name="hobby" value="여행">여행 <input
					...
					<!-- 전체선택 체크박스 클릭 시 체크상태(true, false) 를 함수에 전달 -->
					<!-- name명과 함수명과 동일하면 함수 실행 안 됨! -->
					<input
					type="checkbox" name="check_all" value="전체선택" onclick="checkAll(this.checked)">전체선택
				</td>
			</tr>
			<tr>
				<td>가입동기</td>
			...
			<tr>
				<td colspan="2"><input type="submit" value="가입"> <input
					type="reset" value="초기화"> <input type="button" value="돌아가기"
					onclick="func1()"></td>
			</tr>
		</table>
	</form>
